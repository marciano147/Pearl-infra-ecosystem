...
- Capture the upstream Pearl surfaces that matter for wallets, explorers, payments, OTC tooling, data anchoring, and AI compute marketplace work.
- Define adapters and contracts before product implementation so wallet/explorer/payment apps share the same primitives.
...
1. **Repo role: infrastructure foundation, not first product app**
   ...
   - Rationale: Pearl needs common building blocks before separate wallet/explorer/pay/compute apps diverge.
...
4. **Indexer-first app enablement**
   - Decision: explorer/indexer API should be prioritized alongside chain/wallet adapters because most later apps need address history, confirmations, mempool state, miner data, and OP_RETURN/data lookup.
...
5. **Wallet SDK before extension wallet**
   ...
   - Rationale: extension value depends on apps needing connect/sign. SDK primitives are useful immediately for Pearl Pay, explorer, and future wallets.
...
- [Explorer/indexer scope creep] → Start with read-only blocks/tx/address/mempool/miner APIs before rich analytics.
...
5. Split product apps into separate repos only after infra contracts stabilize.
...
- Which first app should validate the infra: explorer/indexer, Pearl Pay, OTC market app, or compute marketplace?
